...
def test_contact_probability_matches_naive_vectorized_form() -> None:
    # The streaming implementation must be byte-identical to the dense
    # `(distmat < threshold).mean(0)` form it replaced (the dense form OOMs
    # on large chains, hence the per-frame loop). Lock that equivalence so a
    # future "optimization" can't silently shift the validated metrics.
    xyz = np.random.default_rng(3).standard_normal((40, 30, 3)).astype(np.float32)
    naive = (np.linalg.norm(xyz[:, None, :] - xyz[:, :, None], axis=-1) < 0.8).mean(0)
    np.testing.assert_array_equal(contact_probability(xyz, 0.8), naive)


def test_contact_probability_is_fraction_of_frames_in_contact() -> None:
    # Two residues, half the frames in contact (0.2 nm) and half far apart
    # (5 nm): probability is exactly 0.5 off-diagonal, 1.0 on the diagonal.
    close = np.array([[0.0, 0.0, 0.0], [0.2, 0.0, 0.0]])
    far = np.array([[0.0, 0.0, 0.0], [5.0, 0.0, 0.0]])
    xyz = np.stack([close, close, far, far])
    prob = contact_probability(xyz, contact_threshold_nm=0.8)
    assert prob.shape == (2, 2)
    np.testing.assert_allclose(prob, [[1.0, 0.5], [0.5, 1.0]])
