...
def test_chain_page_unknown_chain_returns_404(client: TestClient) -> None:
    r = client.get("/chain/not_a_real_chain_X")
    assert r.status_code == 404


def test_chain_page_known_but_uncached_returns_200(client: TestClient) -> None:
    # The chain page itself doesn't touch the bundle (only the API does);
    # so a val-chain without a cached bundle still renders.
...
def test_topology_endpoint_uncached_returns_503(client: TestClient) -> None:
...
def test_ensemble_endpoint_uncached_returns_503(client: TestClient) -> None:
