...
_SPLIT_CSVS = {"val": _VAL_CSV, "test": _TEST_CSV}
...
def load_chain_sequences(split: str) -> dict[str, str]:
    """Return `{chain: seqres}` for an ATLAS split ('val' or 'test').

    Reads the same vendored split CSV as `load_val_chains` / `load_test_chains`
    but keeps the `seqres` column, which sequence-conditioned samplers and MSA
    generation need. Insertion order matches the CSV.

    Args:
        split: Either `'val'` or `'test'`.

    Returns:
        Ordered mapping of chain id (e.g. `6cka_B`) to its one-letter residue
        sequence.
    ...
    """
    try:
        csv_name = _SPLIT_CSVS[split]
    except KeyError:
        raise ValueError(f"unknown split {split!r}; expected 'val' or 'test'") from None
    csv_text = resources.files(__package__).joinpath(csv_name).read_text(encoding="utf-8")
    reader = csv.DictReader(csv_text.splitlines())
    return {row["name"]: row["seqres"] for row in reader}
...
def view_ensemble_path(chain: str, kind: str, cache_dir: Path | None = None) -> Path:
    """Cache path for the frame-trimmed reference ensemble PDB streamed to the viewer.

    Sits beside the reference observables `.npz`
    (`{cache_dir}/references/{kind}/{chain}.npz`) as
    `{cache_dir}/references/{kind}/{chain}.view.pdb`.
    """
    root = cache_dir or default_cache_dir()
    return root / "references" / kind / f"{chain}.view.pdb"


def load_view_ensemble_pdb_bytes(
    chain: str,
    *,
    kind: str = "analysis",
    cache_dir: Path | None = None,
    ...
    """Return the subsampled reference ensemble PDB, built and cached on first call.

    The viewer streams this for the left (reference) pane. Building it is
    expensive: `load_ensemble_pdb_bytes` extracts three replica XTCs, loads
    ~3000 frames via mdtraj, subsamples, and re-serializes, which on a
    read-only deployment recurs on every request. So the result is cached as a
    multi-model PDB beside the reference `.npz` and reused, mirroring the
    per-sample `_view` cache in `data.samples`. Cached by `(chain, kind)` only;
    `max_frames` is honored on the first (building) call.
    ...
    Args:
        ...
        force: Rebuild even if the view PDB is already cached.
    ...
    Raises:
        FileNotFoundError: If the chain bundle is not cached locally.
    """
    cache = view_ensemble_path(chain, kind, cache_dir)
    if cache.exists() and not force:
        return cache.read_bytes()
    data = load_ensemble_pdb_bytes(chain, kind=kind, cache_dir=cache_dir, max_frames=max_frames)
