...
- AlphaFlow / ESMFlow: HuggingFace zips of multi-model PDBs (one per
  target). Extracted and renamed; no coordinate conversion.
...
PUBLISHED_SOURCES: dict[str, PublishedSource] = {
    "alphaflow_md_base": PublishedSource(
        "alphaflow_md_base", f"{_HF_SAMPLES}/alphaflow_md_base_202402.zip", "zip", "pdb"
    ),
    "esmflow_md_base": PublishedSource(
        "esmflow_md_base", f"{_HF_SAMPLES}/esmflow_md_base_202402.zip", "zip", "pdb"
    ),
    "alphaflow_pdb_base": PublishedSource(
        "alphaflow_pdb_base", f"{_HF_SAMPLES}/alphaflow_pdb_base_202402.zip", "zip", "pdb"
    ),
    "esmflow_pdb_base": PublishedSource(
        "esmflow_pdb_base", f"{_HF_SAMPLES}/esmflow_pdb_base_202402.zip", "zip", "pdb"
    ),
    "eba": PublishedSource(
