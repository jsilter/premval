...
def contact_probability(
    ...
    """Per-residue-pair CA-CA contact probability across an ensemble.

    Args:
        xyz: CA coordinates `(n_frames, n_residues, 3)` (nm).
        contact_threshold_nm: CA-CA distance below which a pair is "in
            contact" in a given frame.

    Returns:
        An `(n_residues, n_residues)` matrix whose `[i, j]` entry is the
        fraction of frames in which residues `i` and `j` are within
        `contact_threshold_nm`.

    The pairwise distances are accumulated one frame at a time. The
    vectorized `xyz[:, None] - xyz[:, :, None]` form materializes an
    `(n_frames, n_residues, n_residues, 3)` tensor that reaches several GB
    on the largest ATLAS chains (~1.9 GB at 400 residues / 1000 frames) and
    OOMs; per frame the intermediate is only `(n_residues, n_residues, 3)`.
    """
    n_frames, n_residues, _ = xyz.shape
    counts = np.zeros((n_residues, n_residues), dtype=np.float64)
    for frame in xyz:
        distmat = np.linalg.norm(frame[:, None, :] - frame[None, :, :], axis=-1)
        counts += distmat < contact_threshold_nm
    counts /= n_frames
    return counts
...
def contact_jaccard(
    ...
    *,
    ...
    ref_prob = contact_probability(ref_arr, contact_threshold_nm)
    sub_prob = contact_probability(sub_ca_xyz, contact_threshold_nm)
    crystal_distmat = np.linalg.norm(crystal_ca_xyz[None, :] - crystal_ca_xyz[:, None], axis=-1)
